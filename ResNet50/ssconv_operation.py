...
class ScConv(nn.Module):
    def __init__(self, op_channel:int,stride, group_num:int = 16, gate_treshold:float = 0.5, alpha:float = 1/2, squeeze_radio:int = 2, group_size:int = 2, group_kernel_size:int = 3):
        super().__init__()  # 调用父类构造函数
...
if __name__ == '__main__':
    x       = torch.randn(16, 32, 16, 16)  # 创建随机输入张量
    model   = ScConv(32,2)  # 创建 ScConv 模型
