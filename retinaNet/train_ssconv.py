...
import transforms
from backbone import resnet50_ssconv_fpn_backbone, LastLevelP6P7
...
def create_model(num_classes):
    ...
    backbone = resnet50_ssconv_fpn_backbone(norm_layer=torch.nn.BatchNorm2d,
    ...
    for key, _ in weights_dict.items():
        if "conv2" in key:
            del_keys.append(key)
    keys_to_add = ["head.classification_head.cls_logits.weight", "head.classification_head.cls_logits.bias"]
    del_keys.extend(keys_to_add)
...
def count_parameters(model,device):
    ...
    input_size = (1, 3, 224, 224)  # 假设的输入大小
...
if __name__ == "__main__":
    ...
    # 训练数据集的根目录(VOCdevkit)
    parser.add_argument('--data-path', default=current_dir, help='dataset')
    # 检测目标类别数(不包含背景)
